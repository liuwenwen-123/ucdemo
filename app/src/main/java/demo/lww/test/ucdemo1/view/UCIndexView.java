package demo.lww.test.ucdemo1.view;
...
public class UCIndexView extends FrameLayout implements TouchMoveView.TouchMoveListener {
     ...
     * 内容视图的头部视图是否启用，默认启用
     */
    private boolean mIsContentHeadViewEnable = false;
    /**
...
//        step * mContentHeadView.getNeedMoveHeight() / mContentView.getNeedMoveHeight()
        float contentHeadViewStep = mIsContentHeadViewEnable ? step : 0;
...
//                    mContentHeadView.onShowAnimation(contentHeadViewStep);
    ...
    private boolean isHideFinish() {
        ...
        boolean pageHeadViewHideFinish = mIsPageHeadViewFixed ? true : true;
        //当ContentHeadView不启用的时候，默认为恢复结束
        boolean contentHeadViewHideFinish = mIsContentHeadViewEnable ? true : true;
    ...
    private boolean isShowFinish() {
        ...
        boolean pageHeadViewShowFinish = mIsPageHeadViewFixed ? true : true;
        //当ContentHeadView不启用的时候，默认为展示结束
        boolean contentHeadViewShowFinish = mIsContentHeadViewEnable ? true : true;
